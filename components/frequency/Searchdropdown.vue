...
function onSearch(evt) {
    ...
    ref_freqStore.filterFrequencies(searchText)
}

function onSelect(filter) {
    console.log(filter);
    
    emit('selected-value', filter)
}
...
async function fetchfrequencies() {
    await ref_freqStore.fetchfrequencies()
}
...
<template>
    <button id="dropdownSearchButton" data-dropdown-toggle="dropdownSearchClass"
    ...
    <div id="dropdownSearchClass" class="z-10 hidden bg-white rounded-lg shadow w-96 dark:bg-gray-700">
        ...
        <ul class="h-48 px-3 pb-3 overflow-y-auto text-sm text-green-700 dark:text-green-200"
            ...
            <li v-for="data in frequencyData" :key="data.id">
                <div class="flex items-center ps-2 rounded hover:bg-gray-100 dark:hover:bg-gray-600">
                    <input type="radio" :value="data.attributes.Time" v-model="filterData.selectedFrequency.val" @click="onSelect(data)"
